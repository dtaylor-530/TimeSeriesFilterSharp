...
namespace TimeSeriesFilter.Terminal
{
    public class MainViewModel: INPCBase
    {
        ...
        public MainViewModel( DispatcherService ds,Dispatcher dispatcher)
        {

            var tss = new TimeSeriesService();

            TimeSeriesController tsc = new TimeSeriesController(tss.Measurements,dispatcher);
            ...
            FluxInputVM.Output.DistinctUntilChanged().Subscribe(_ =>
            {
                ...
                if (_ == Flux.Static)
                {
                     ...
                     prds = new PredictionsController((prrds as PredictionsService).Predictions, ds.UI,dispatcher);
                }
