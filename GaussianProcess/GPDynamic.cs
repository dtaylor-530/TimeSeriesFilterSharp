...
namespace GaussianProcess
{
    ...
    {
        public static GPDynamic Build(Type type, double length, double noise)
        {

            return (new GPDynamic((IMatrixkernel)Activator.CreateInstance(type), length, noise));
        }
    ...
    public class GPDynamic
    {
        ...
        public Svd<double> Update(double PointsX, double PointsY)
        {

            TrainingPointsX.Add(PointsX);
            TrainingPointsY.Add(PointsY);

            var trarr = TrainingPointsX.ToArray();

            Matrix<double> dmTr = MathHelper.ComputeDistanceMatrix(trarr, trarr);

            Vector<double> v = Vector<double>.Build.DenseOfArray(TrainingPointsY.ToArray());

            var Kxx_p_noise = kernel.Main(dmTr, length);
            for (int i = 0; i < TrainingPointsX.Count; i++)
                Kxx_p_noise[i, i] += noise;
